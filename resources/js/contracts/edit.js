...
$(document).ready(function () {
    ...
    // If have equipment
    window.checkIfHaveEquipment(document.querySelector("#chk-have-equipment"));
    ...
    $(["#receipt-amount", "#contract-value", "#contract-payment-cash"]).mask("#.##0,00", { reverse: true });
...
window.fillInAddressFields = async function (el) {
    ...
    const clientNumber = document.querySelector(`#${elemItem}-number`);
    ...
    const addressFields = [
        el, address, clientNumber, complement, neighborhood, city, state
    ];
...
// Show inputs if have equipment
window.checkIfHaveEquipment = function (el) {
    const equipmentInformations = document.querySelector("#equipment-informations");
    const equipmentInputs = document.querySelectorAll(`#${equipmentInformations.id} input`);

    if (el.checked) equipmentInformations.classList.remove("d-none");

    else {
        equipmentInformations.classList.add("d-none");

        equipmentInputs.forEach(input => {
            const feedback = input.closest(".form-group").lastElementChild;
...
window.validateDateAcquisition = function (el) {
    ...
    const delivery = document.querySelector("#equipment-delivery-date");
    const deliveryDate = new Date(
        delivery.value.split("-")[0], delivery.value.split("-")[1] - 1, delivery.value.split("-")[2]
    );
    const dateAcquisitionFeedback = document.querySelector("#date-acquisition-feedback");
    const deliveryDateFeedback = document.querySelector("#delivery-date-feedback");
    const minDate = new Date(
        "01/01/2000".split("/")[2], "01/01/2000".split("/")[1] - 1, "01/01/2000".split("/")[0]
    );

    if (el.value.split("-").length < 3 || el.value.split('-')[0].length > 4) {
        dateAcquisitionFeedback.innerText = "Preencha o campo.";
        validate(el, false);
        validateFeedback(dateAcquisitionFeedback, false);
        return false;
    }

    else if (el.value.split("-").join("").length < 8) {
        dateAcquisitionFeedback.innerText = "Data inválida.";
        validate(el, false);
        validateFeedback(dateAcquisitionFeedback, false);
        return false;
    }

    else if (acquisitionDate < minDate || acquisitionDate > todayDate) {
        dateAcquisitionFeedback.innerText = `Digite uma data entre 01/01/2000 e ${toDateFormat(Date.now(), "pt-br")}.`;
        validate(el, false);
        validateFeedback(dateAcquisitionFeedback, false);
        return false;
    }

    else if (deliveryDate < acquisitionDate) {
        dateAcquisitionFeedback.innerText = "Data de Aquisição não pode ser posterior à Data de Entrega.";
        validate(el, false);
        validateFeedback(dateAcquisitionFeedback, false);

        deliveryDateFeedback.innerText = "Data de Entrega não pode ser anterior à Data de Aquisição.";
        validate(delivery, false);
        validateFeedback(deliveryDateFeedback, false);
        return false;
    }

    else {
        dateAcquisitionFeedback.innerText = "Formato aceito.";
        validate(el, true);
        validateFeedback(dateAcquisitionFeedback, true);

        if (delivery.value.length !== 0 && deliveryDate < todayDate) {
            deliveryDateFeedback.innerText = "Formato aceito.";
            validate(delivery, true);
            validateFeedback(deliveryDateFeedback, true);
        }
...
window.validateDeliveryDate = function (el) {
    const today = toDateFormat(Date.now(), "en-us");
    const todayDate = new Date(
        today.split("-")[0], today.split("-")[1] - 1, today.split("-")[2]
    );
    ...
    const acquisition = document.querySelector("#equipment-date-acquisition");
    const acquisitionDate = new Date(
        acquisition.value.split("-")[0], acquisition.value.split("-")[1] - 1, acquisition.value.split("-")[2]
    );
    const dateAcquisitionFeedback = document.querySelector("#date-acquisition-feedback");
    const deliveryDateFeedback = document.querySelector("#delivery-date-feedback");
    const minDate = new Date(
    ...
    if (el.value.split("-").length < 3 || el.value.split("-")[0].length > 4) {
        deliveryDateFeedback.innerText = "Preencha o campo.";
        deliveryDateFeedback.style.display = "block";
        validate(el, false);
        validateFeedback(deliveryDateFeedback, false);
        return false;
    }
        ...
        deliveryDateFeedback.innerText = "Data inválida.";
        validate(el, false);
        validateFeedback(deliveryDateFeedback, false);
        return false;
    }

    else if (deliveryDate < minDate || deliveryDate > todayDate) {
        deliveryDateFeedback.innerText = `Digite uma data entre 01/01/2000 e ${toDateFormat(Date.now(), "pt-br")}.`;
        validate(el, false);
        validateFeedback(deliveryDateFeedback, false);
        return false;
    }
        ...
        if (acquisition.value.length !== 0 && acquisitionDate < todayDate) {
            dateAcquisitionFeedback.innerText = "Formato aceito.";
            validate(acquisition, true);
            validateFeedback(dateAcquisitionFeedback, true);
        }
...
window.generateContract = function () {
    let submit = true;
    const formGenerateContract = document.querySelector("#form-generate-contract");
    const signatureName = document.querySelector("#contract-signature-name");

    const isSignatureName = window.validateSelect(signatureName);

    errorFocus();

    if (!isSignatureName) submit = false;

    if (submit) formGenerateContract.submit();
}

/** Generate Receipt Payment */
window.generateReceiptPayment = function () {
    let submit = true;
    const formGenerateReceiptPayment = document.querySelector("#form-generate-receipt-payment");
    ...
    const isReceiptAmount = window.validateDouble(receiptAmount);
    const isReceiptDescription = window.validateTextarea(receiptDescription, 10, 250);

    errorFocus();

    if (!isReceiptAmount) submit = false;
    if (!isReceiptDescription) submit = false;

    if (submit) formGenerateReceiptPayment.submit();
}
...
/** Create Equipment */
window.submitFormEditContract = async function (el) {
    let submit = true;
...
/** Edit contract */
window.submitFormEditContract = function () {
    let submit = true;
    const formEditContract = document.querySelector("#form-edit-contract");
    const contractSeller = document.querySelector("#contract-seller");
    const contractClient = document.querySelector("#contract-client");
    ...
    const contractArea = document.querySelector("#contract-area");
    const contractMonthlyAvgGeneration = document.querySelector("#contract-monthly-avg-generation");
    const chkHaveEquipment = document.querySelector("#chk-have-equipment");
    const equipmentDateAcquisition = document.querySelector("#equipment-date-acquisition");
    const equipmentDeliveryDate = document.querySelector("#equipment-delivery-date");
    const equipmentFileInvoice = document.querySelector("#equipment-file-invoice");
    ...
    const isValidContractStatus = window.validateSelect(contractStatus);
    ...
    if (!isValidContractStatus) submit = false;
    ...
    if (chkHaveEquipment.checked) {
        const isValidEquipmentDateAcquisition = window.validateDateAcquisition(equipmentDateAcquisition);
        const isValidEquipmentDeliveryDate = window.validateDeliveryDate(equipmentDeliveryDate);
        const isValidEquipmentFileInvoice = window.validateFile(equipmentFileInvoice);

        errorFocus();

        if (!isValidEquipmentDateAcquisition) submit = false;
        if (!isValidEquipmentDeliveryDate) submit = false;
        if (!isValidEquipmentFileInvoice) submit = false;
    }
    ...
    if (submit) {
        ...
        const btnEditContract = document.querySelector("#btn-edit-contract");
        const btnEditContractLoading = document.querySelector("#btn-edit-contract-loading");
        const btnEditContractIcon = document.querySelector(`#${btnEditContract.id} i`);

        btnEditContract.setAttribute("disabled", true);
        btnEditContractIcon.classList.add("d-none");
        btnEditContractLoading.classList.remove("d-none");
