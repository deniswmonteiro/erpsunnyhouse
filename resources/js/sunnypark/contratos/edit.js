...
function initComponents() {
    ...
    const paymentValue = $('#payment_value').data('xpc');
    ...
    // Fix to place the correct payment value
    $('#payment_value').val(paymentValue);
}
...
function autocomplete(inp, arr) {
    ...
    $('#' + inp.id).on('input', function (e) {
        ...
        for (i = 0; i < arr.length; i++) {
            ...
            if (arr[i][0].toLowerCase().includes(val.toLowerCase()) && size < limit) {
                has_items = true;
                size++;
