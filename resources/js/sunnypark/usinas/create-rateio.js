...
window.submit_form_rateio_create = function () {
    submit();
};
...
function submit() {
    ...
    if (!isValidNumber('#rateio', 0)) submit = false;
    if (!isValidNumber('#ciclocreditos', 0)) submit = false;
    if (!isValidInput('#vigencia', 1)) submit = false;
    
    errorFocus();

    if (submit) $("#form-create-rateio").submit();
}