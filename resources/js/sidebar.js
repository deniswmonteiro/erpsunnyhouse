...
document.querySelectorAll('.sidebar-item').forEach(sidebarItem => {
    sidebarItem.querySelectorAll('.sidebar-link').forEach(sidebarLink => {
        // let item = sidebarLink.getAttribute('href').split('/');
        // if(window.location.href.includes(item[item.length-1])) sidebarItem.classList.add('active');
    })
});

// Scroll into active sidebar
document.querySelector('.sidebar-item.active') !== null && document.querySelector('.sidebar-item.active').scrollIntoView(false)
