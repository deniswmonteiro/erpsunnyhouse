...
async function executeAjaxEmail(email) {
    const body = {
        "email": email.value
    };
...
window.validateEmail = async function (el) {
    ...
    if (el.value.length === 0) {
        ...
        const result = await executeAjaxEmail(el);
...
window.submitFormUserCreate = async function () {
    let submit;
    ...
    if (chkIsEngineer.checked || userCategory.selectedIndex === 2) {
    ...
    if (submit) formCreateUser.submit();
}