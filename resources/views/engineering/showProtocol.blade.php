...
<div class="d-none" id="protocol-form-{{$type}}-{{$key + 1}}-{{$gen_key + 1}}">
    <form action="{{route('engineering_protocol', ['type' => encrypt($type), 'id' => encrypt($generator->id)])}}"
        method="POST" enctype="multipart/form-data"
        id="form-protocol-{{$type}}-{{$key + 1}}-{{$gen_key + 1}}"
        onsubmit="return window.submitFormProtocol(this)">
        @csrf
        <!-- Protocol Number -->
        @if ($type != "homologated")
            <div class="row">
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control" type="text"
                            ...
                            @if ($generator->$type != null)
                                value="{{$generator->$type->protocol_number}}"
                            @endif
                ...
                <div class="form-group">
                    ...
                    <div class="input-group">
                        <input class="form-control date" type="date"
                            ...
                            @if ($generator->$type != null)
                                value="{{$generator->$type->protocol_date}}"
                            @endif
                            min="2000-01-01"
    ...
    <div class="row">
        <div class="col-12 mb-1">
            <div class="form-group">
                ...
                <p>
                    @if ($generator->$type != null && $type != 'homologated')
                        {{$generator->$type->protocol_number}}
                    @else
                        &mdash;
                    @endif
                </p>
    ...
    <div class="row">
        <div class="col-12 mb-1">
            <div class="form-group">
                ...
                <p>
                    @if ($generator->$type != null)
                        {{date('d/m/Y', strtotime($generator->$type->protocol_date))}}
                    @else
                        &mdash;
                    @endif
                </p>
