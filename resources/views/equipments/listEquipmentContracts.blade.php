@section('page_title', 'Contratos Relacionados com o Equipamento')

<script src="{{asset(mix('js/equipments/listEquipmentContracts.js'))}}" defer></script>
...
<x-app-layout>
    ...
    <div id="equipment-contracts-list">
        <div class="card">
                ...
                <div class="row">
                    <div class="col-12">
                        ...
                        <p class="card-description">
                            Os seguintes contratos possuem o equipamento <span class="fw-bold text-primary">{{$equipment->name}}</span>
                        </p>
                    </div>
                </div>
            </div>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @switch ($contract->status)
                                                @case ("ORÇANDO")
                                                    <span class="badge bg-secondary fw-bold">{{$contract->status}}</span>
                                                    @break
                                                
                                                @case ("CONTRATADO")
                                                    <span class="badge bg-brown fw-bold">{{$contract->status}}</span>
                                                    @break

                                                @case ("ATIVO")
                                                    <span class="badge bg-info fw-bold">{{$contract->status}}</span>
                                                    @break
                                                
                                                @case ("PENDÊNCIA")
                                                    <span class="badge bg-danger fw-bold">{{$contract->status}}</span>
                                                    @break

                                                @case ("INSTALANDO")
                                                    <span class="badge bg-primary fw-bold">{{$contract->status}}</span>
                                                @break

                                                @case ("INSTALADO")
                                                    <span class="badge bg-warning fw-bold">{{$contract->status}}</span>
                                                @break

                                                @case ("CONCLUÍDO")
                                                    <span class="badge bg-success fw-bold">{{$contract->status}}</span>
                                                @break

                                                @case ("CANCELADO")
                                                    <span class="badge bg-black fw-bold">{{$contract->status}}</span>
                                                @break
                                            @endswitch
                                        </td>
